...
def run_simulation(bankroll, games):
    ...
    for round_num in range(1, games + 1):  # Run the number of specified rounds
        ...
        d = round(random.uniform(1, 53))
        n = round(random.uniform(1, (d + 1)/2))
        odds = round(random.uniform(0.5, 13.0), 1)
        p = float(n)/float(d)
        
        # Display the randomly generated probabilities and odds 
        print(f"Round {round_num}: Probability: {n}/{d}, Odds: {odds}:1")
        ...
        kelly_bet = calculate_kelly_bet(p, odds, bankroll)  # Use Kelly bankroll
        print(f"Optimal Kelly Bet: ${kelly_bet:.2f}")
        adversary_bet = calculate_kelly_bet(p, odds, kelly_winnings[-1])  # Use Kelly bankroll
        
        # Simulate the user bet
        user_result, kelly_result = simulate_bet(p, odds, bet, adversary_bet)
        ...
        print(f"Adversary Bankroll: ${kelly_bankroll:.2f}\n")
